...
const User = ({ user, settings = false }: LayoutProps) => {
  ...
  const handleDismiss = useCallback(() => {
    if (settingsPage) router.replace(`/${user.email}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const descMdx = await response.json();
        setData({
          ...data,
          descMdx
        }); // optimistically show updated state for descMdx
        router.replace(`/${user.email}`, undefined, { shallow: true });
  ...
  return (
    <div className="min-h-screen pb-20">
          ...
          <div className="mt-6 sm:flex-1 sm:min-w-0 sm:flex sm:items-center sm:justify-end sm:space-x-6 sm:pb-1">
            ...
            {user.emailVerified ? (
              <div className="mt-6 flex flex-col justify-stretch space-y-3 sm:flex-row sm:space-y-0 sm:space-x-4">
                <a
                  href={`https://github.com/${user.email}`}
                  target="_blank"
              ...
              <div className="mt-6 flex flex-col justify-stretch space-y-3 sm:flex-row sm:space-y-0 sm:space-x-4">
                <a
                  href="https://github.com/madmax6266"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex justify-center px-4 py-2 border border-gray-800 hover:border-white shadow-sm text-sm font-medium rounded-md text-white font-mono bg-black focus:outline-none focus:ring-0 transition-all"
                >
      ...
      <div className={`${profileWidth} mt-16`}>
        <h2 className="font-semibold font-mono text-2xl text-white">Desc</h2>
        {settingsPage ? (
          <>
            <TextareaAutosize
              name="description"
              ...
              placeholder="Enter a short desc about yourself... (Markdown supported)"
              value={data.desc}
            />
            <div className="flex justify-end w-full max-w-2xl">
              <p className="text-gray-400 font-mono text-sm">
                {data.desc.length}/256
              </p>
            </div>
          </>
        ) : (
